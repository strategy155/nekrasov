...
export class MarkdownSerializer {
  ...
  constructor(
    ...
    // This serializer is used by table for example, and is passed from the outside
...
/**
 * This function is taking a PM Node, contaning table (the mapping is done weirdly by function name, sorry for that),
 *  and converting it to an HTMLElement, which is later written to a state
 * @param state , a state of a serializer, which is being written
 * @param tableNode , a table PM Node instance
 */
function table(state: MarkdownSerializerState, tableNode: Node) {
  // serializing a table node to an html element
  const tableHTMLRepresentation = state.DOMSerializer.serializeNode(
    tableNode
  ) as HTMLElement;

  // using outerHTML attribute to get a string of the contents of the node
  state.write(tableHTMLRepresentation.outerHTML);

  // making a favour and adding a newline symbol
  state.write();
}
...
export class MarkdownSerializerState {
  ...
  /// @internal
  constructor(
    // We need that serializer to serialize some complicated nodes, such as table
    // IT is passed fro the parent serializer, to this state instance
    /// @internal
    readonly DOMSerializer: DOMSerializer,
    /// @internal
    readonly nodes: {
